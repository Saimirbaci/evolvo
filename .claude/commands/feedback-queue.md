---
description: List pending feedback rows from the local workspace (~/.evolvo/evolvo_workspace/feedback) and propose the top 5 to work, ranked.
---

Read feedback JSON from `${EVOLVO_WORKSPACE_ROOT:-$HOME/.evolvo/evolvo_workspace}/feedback/`. For each file, extract `id`, `status`, `feedbackType`, `pageRoute`, first 120 chars of `feedbackText`, and `createdAtUnixMs`.
